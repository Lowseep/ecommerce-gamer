...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h2 class="text-white font-bold text-lg">Nuevo Producto</h2>
            <p class="text-gray-500 text-xs">Completa los campos para agregar un producto al catálogo</p>
        </div>
    </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-4 md:p-6 mb-4">
            ...
            <div class="mt-4">
                ...
                <textarea name="descripcion" id="inputDescripcion" rows="4" required
                          ...
                          class="w-full bg-gray-800 border border-gray-700 hover:border-gray-600 focus:border-cyan-500 rounded-xl px-4 py-3 text-white text-sm placeholder-gray-600 focus:outline-none transition resize-none">{{ old('descripcion') }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="precio" id="inputPrecio" value="{{ old('precio') }}"
                    ...
                    <p id="msgPrecio" class="text-xs mt-1 text-gray-600 flex items-center gap-1">
                        <i class="fas fa-info-circle"></i> Ingresa un precio mayor a 0
                    </p>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="stock" id="inputStock" value="{{ old('stock') }}"
                            placeholder="0"
                            min="0" required
                    ...
                    <p id="msgStock" class="text-xs mt-1 text-gray-600 flex items-center gap-1">
                        <i class="fas fa-info-circle"></i> Stock en 0 deshabilita la compra automáticamente
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-4 md:p-6 mb-6">
            ...
            <div class="border-2 border-dashed border-gray-700 hover:border-cyan-600 rounded-xl p-4 md:p-6 text-center transition cursor-pointer"
                ...
                <div id="uploadIcon">
                    ...
                    <p class="text-gray-400 text-sm font-medium">Haz click para subir una imagen</p>
...
document.getElementById('inputPrecio').addEventListener('keypress', function(e) {
    if (!/[0-9.]/.test(e.key) && e.key !== 'Backspace' && e.key !== 'Delete' && e.key !== 'Tab') {
        e.preventDefault();
        setMsg('msgPrecio', 'error', 'Solo se permiten números.');
        marcarCampo('inputPrecio', 'error');
        setTimeout(() => {
            if (this.value !== '') validarPrecio();
            else { marcarCampo('inputPrecio','default'); setMsg('msgPrecio','hint','Solo números.'); }
        }, 1500);
    }
});
...
function validarPrecio() {
    ...
    if (isNaN(val) || document.getElementById('inputPrecio').value === '') {
        ...
        setMsg('msgPrecio','hint','Solo números. Ej: 120.00');
        return false;
    }
    if (val <= 0) {
        marcarCampo('inputPrecio','error');
        setMsg('msgPrecio','error','El precio debe ser mayor a 0');
        return false;
    }
    marcarCampo('inputPrecio','ok');
    setMsg('msgPrecio','ok','Precio válido: S/ ' + val.toFixed(2));
    return true;
}

// ── STOCK — solo números enteros, bloquea letras en tiempo real
document.getElementById('inputStock').addEventListener('input', function() {
    const original = this.value;
    this.value = this.value.replace(/[^0-9]/g, '');

    if (original !== this.value) {
        setMsg('msgStock','error','Solo se permiten números enteros, sin letras ni símbolos');
...
function validarStock() {
    ...
    if (val === '') {
        marcarCampo('inputStock','default');
        setMsg('msgStock','hint','Solo números enteros. Ej: 10');
        return false;
    }
    if (parseInt(val) < 0) {
        marcarCampo('inputStock','error');
        setMsg('msgStock','error','El stock no puede ser negativo');
        return false;
    }
    marcarCampo('inputStock','ok');
    setMsg('msgStock','ok','Stock válido: ' + val + ' unidades');
    return true;
}
